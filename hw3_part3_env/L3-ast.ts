...
export const makeVarDecl = (v: string, lazy?: boolean): VarDecl => ({
	tag: "VarDecl",
	var: v,
...
export const isArray = (x: any): x is Array<any> => x instanceof Array;
...
const parseDefine = (es: any[]): DefineExp | Error =>
	(es.length !== 2) ? Error(`define should be (define var val) - ${es}`) :
		!isString(es[0]) ? Error(`Expected (define <var> <CExp>) - ${es[0]}`) :
			safeF((val: CExp) => makeDefineExp(makeVarDecl(es[0], false), val))(parseL3CExp(es[1]));
...
{
	const newParseDecls = (sexp: any): VarDecl =>
	{
		if (isArray(sexp))
		{
			if (sexp[1] === "lazy")
				return makeVarDecl(first(sexp), true);
			else
				Error("No Lazy")
		}
		return makeVarDecl(sexp, false);
	};
	return safeFL((body: CExp[]) => makeProcExp(map(newParseDecls, sexps[1]), body))
