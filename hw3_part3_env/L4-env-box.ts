...
import {L4applicativeEval} from "./L4-eval-box";
...
export const makeThunk = (_arg: CExp4, _env: Env): thunk =>
	({tag: "thunk", arg: _arg, env: _env});
...
export const getFBindingVal = (f: FBinding): Value4 | Error =>
{
	let v: Value4 | thunk = unbox(f.val);
	return isThunk(v) ? L4applicativeEval(v.arg, v.env) : v
};
export const setFBinding = (f: FBinding, val: Value4): void =>
{
	setBox(f.val, val);
	return;
};
...
const makeFrame = (vars: string[], vals: (Value4 | thunk)[]): Frame =>
...
export const makeExtEnv = (vs: string[], vals: (Value4 | thunk)[], env: Env): ExtEnv =>
...
const applyExtEnvBdg = (env: ExtEnv, v: string): FBinding | Error =>
{
	...
	return isError(bdg) ? applyEnvBdg(env.env, v) : bdg;
};
