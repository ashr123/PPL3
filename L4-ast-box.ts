...
const parseProcExp4 = (sexps: any[]): ProcExp4 | Error =>
	safeFL((body: CExp4[]) => makeProcExp4(map((val: any) => isArray(val) ? (val.length === 2 && val[1] === "lazy" ? makeVarDecl(val[0], true) : Error("Not lazy")) : makeVarDecl(val, false), sexps[1]), body))(map(parseL4CExp, rest(rest(sexps))));
