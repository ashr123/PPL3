							...
							isLitExp(exp) ? exp.val :
								isIfExp(exp) ? evalIf(exp, env) :
									isProcExp(exp) ? evalProc(exp, env) :
										isAppExp(exp) ? L3applyProcedure(L3applicativeEval(exp.rator, env), exp.rands, env) :
...
const valueToLitExp = (v: Value | Error): NumExp | BoolExp | StrExp | LitExp | PrimOp | ProcExp | Error =>
	isError(v) ? v :
