...
assert.deepEqual(evalParse(`
    (L3 ((lambda ((x lazy)) x) 10))`), 10);

assert.deepEqual(evalParse(`
    (L3 (if ((lambda(l (y lazy) z)((lambda ((x lazy)) (= x 10)) 10)) 1 (/ 0 0) 2) ((lambda ((m lazy)) m) (* 6 6)) ((lambda (v b (h lazy)) (3))(1 2 3))))`),
	36);

// Yekir Green test
assert.deepEqual(evalParse(`
(L3 (define loop (lambda ((x lazy)) 6)) (define loop2 (lambda ((y lazy)) (loop2 y))) ((lambda (f) f) (loop (loop2 0))))`),
	6);

// Shaked A test
assert.deepEqual(evalParse(`
(L3 (define loop (lambda (x) (loop x))) (define square (lambda ((x lazy)) 1)) (define bla (lambda (x) square)) ((bla 3) (loop 5)))`),
	1);

// Yehonatan Peleg tests (modified)
assert.deepEqual(evalParse(`
(L3 (((lambda ((a lazy)) (lambda ((b lazy)) 5)) 7) (g 5 6)) )`),
	5);
assert(isError(evalParse(`
(L3 (((lambda (a) (lambda (b) 5)) 7) (g 5 6)) )`)));
...
import {
	isAppExp, isBoolExp, isCExp, isDefineExp, isIfExp, isLetExp, isLitExp, isNumExp, isPrimOp,
	isProcExp, isProgram, isStrExp, isVarDecl, isVarRef
} from './L3-ast';
import {renameExps, substitute} from './L3-eval';
